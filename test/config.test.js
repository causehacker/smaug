...
describe('loadConfig', () => {
  ...
  test('default source is bookmarks', () => {
    const config = loadConfig('./nonexistent.json');
    assert.strictEqual(config.source, 'bookmarks');
  });

  test('default includeMedia is false', () => {
    const config = loadConfig('./nonexistent.json');
    assert.strictEqual(config.includeMedia, false);
  });
});
